...
/**
 * This class contains the configuration options used to download from the file share service using smart retries,
 * starting from a checkpoint, rather than starting over from the beginning.
 */
@Fluent
public final class DownloadRetryOptions {
     ...
     */
    private int maxRetryRequests = 1; // change this to 1 to trigger failure
    ...
    public DownloadRetryOptions setMaxRetryRequests(int maxRetryRequests) {
        if (maxRetryRequests < 0) {
            throw LOGGER.logExceptionAsError(new IllegalArgumentException(
                String.format(Locale.ROOT, PARAMETER_NOT_IN_RANGE, "options.maxRetryRequests", 0, Integer.MAX_VALUE)));
        }
